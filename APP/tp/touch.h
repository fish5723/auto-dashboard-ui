...
typedef struct
{
    ...
    uint8_t (*scan)(uint8_t);       // 扫描触摸屏. 0:屏幕坐标扫描; 1:物理坐标扫描(校准用)
    void (*adjust)(void);           // 触摸屏校准

    uint16_t x[5];                  // 当前坐标 x[0]:当前扫描坐标; x[4]:第一次按下时的坐标
    uint16_t y[5];                  // 当前坐标 y[0]:当前扫描坐标; y[4]:第一次按下时的坐标
    uint8_t  sta;                   // 笔的状态
    // b7:按下1/松开0;
    // b6:0,没有按键按下;1,有按键按下.
    // b5:保留
    ...
    // 触摸类型参数
    // b0: 0,竖屏(左右为X,上下为Y); 1,横屏(左右为Y,上下为X)
    // b1~6: 保留
    // b7: 0,电阻屏; 1,电容屏
    uint8_t touchtype;
}_m_tp_dev;
...
void load_draw_hint(void); //触摸屏初始化
void rtp_test(void);       //电阻屏测试
